...
tim.microMove(5);
	...
	private void spaceBarWasPressed() {
		...
		int robotXLocation = tim.getX();
		int robotYLocation = tim.getY();
		...
		if(robotXLocation==730 && robotYLocation==415) { JOptionPane.showMessageDialog(null, "Go to treehouse");}
		// 8. Give the user subsequent clues at different locations on the image
		// (pirate robot, swamp, parrots, etc.)
		if(robotXLocation==245 && robotYLocation==215) { JOptionPane.showMessageDialog(null, "Go to message in bottle");}
		// 9.  If the robot is in the final location
		//     --call the treasureFound() method
		if(robotXLocation==415 && robotYLocation==510) { treasureFound();}
	}
